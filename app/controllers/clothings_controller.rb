class ClothingsController < ApplicationController
  ...
  def index
    ...
    @clothings = @clothings.where(["name LIKE :filter", :filter => ("%" + params[:filter] + "%")]) unless params[:filter].blank?


    @clothings = @clothings.order(:brands).page(params[:page] || 1)
  end
  ...
  private
    ...
    def clothing_params
      params.require(:clothing).permit(:name, :color, :description, :brand_id, :type_id)
    end
end
